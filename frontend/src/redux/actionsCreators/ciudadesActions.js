...
const ciudadesActions = {                                 //las Actions es un solo objeto que va contener acciones!!
    ...
    fetchearCiudades: () => {                           //cada llamado va a ser una función
        return async(dispatch, getState)=>{             //la Fx asyncrona va dentro de un return en este caso de las actions
            const res = await axios.get('https://mytinerary-daniel-martinez.herokuapp.com/api/allcities')   //guardo mis ciudades en res
    ...
    fetchearUnaCiudad: (id) => {                                               //llega el id como parametro de la ciudad
        return async(dispatch, getState) => {
            try{
                const res = await axios.get('https://mytinerary-daniel-martinez.herokuapp.com/api/cities/'+id)
    ...
    borrarCiudad: (id)=>{
        return async(dispatch, getState)=>{
            try {
                const res = await axios.delete('https://mytinerary-daniel-martinez.herokuapp.com/api/allcities/'+id)
    ...
    cargarCiudad: (name, descripcion)=>{
        return async(dispatch, getState)=>{
            const res = await axios.post('https://mytinerary-daniel-martinez.herokuapp.com/api/allcities',{name,descripcion})
